...
3. This software (Schwarzschild Metric Tensor Calculator):
   @software{EinsteinKPy2Tex2026,
       author = {P. Ntelis},
       title = {EinsteinKPy2Tex: Schwarzschild Metric Tensor Calculator},
       year = {2026},
...
def print_citation():
    ...
    [3] P. Ntelis (2026). EinsteinKPy2Tex: Schwarzschild Metric Tensor 
...
def generate_bib_file():
    ...
    bib_content = """% BibTeX citations for EinsteinKPy2Tex project
...
R_s = sp.Symbol("R_s", positive=True, real=True)  # Schwarzschild radius R_s = 2 G M / c^2
...
def compute_geodesic_equations(christoffel, coords):
    ...
    # Create symbols for Christoffel symbol evaluation
...
def metric_inv_tensor(metric, coords):
    """Compute inverse metric tensor"""
    metric_matrix = sp.Matrix(metric)
    inv_metric_matrix = metric_matrix.inv()
    return sp.Array(inv_metric_matrix)
...
with open(filename, "w") as f:
    ...
    f.write("\\item P. Ntelis (2026). EinsteinKPy2Tex: Schwarzschild Metric Tensor Calculator (Version 1.0).\n")
    ...
    f.write("\n\\end{align}\n\n")
    f.write("\\subsubsection{Simplified expression from code}\n")
    f.write("\\begin{align} K = \n")
    f.write(sp.latex(kretschmann_simplified))
    f.write("\n\\end{align}\n\n")
    f.write("\\textbf{Note:} The simplified expression should equal the analytic result from the literature, i.e. $K = \\frac{48 G^2 M^2}{c^4 r^6} = \\frac{12 R_s^2}{r^6}$. ")
    f.write("If they do not match, check the Riemann tensor index ordering.\n\n")
